...
//       : phi::funcs::OneDNNHandlerNoCachingT<T_in,
//       dnnl::inner_product_forward>(
...
//       post_operations.append_eltwise(dnnl::algorithm::eltwise_relu, 0.0f,
//       0.0f);
//     }
...
//     target_memory_p = std::make_shared<dnnl::memory>(target_md,
//     this->engine_); auto reorder_p = std::make_shared<dnnl::reorder>(
...
//     if (x->dims().size() != 2) {
//       // reshape restrictions are always satisfied because in case of 3 or 4
//       dim
...
//                           "Weight padding in fc can not be used in
//                           oneDNN."));
